...
- `fibers` (integer) executes jobs as that many fibers on a single reactor thread (using the [async](https://github.com/socketry/async) gem) instead of a thread pool. Fibers cooperatively yield during IO, so IO-bound workloads can run at much higher concurrency with a fraction of the OS threads and database connections; CPU-bound work will not benefit, and blocking C extensions that don't cooperate with Ruby's fiber scheduler will stall the reactor. Requires Ruby 3.2+, the `async` gem (>= 2.24) in your Gemfile, `config.active_support.isolation_level = :fiber`, and code reloading disabled (`config.enable_reloading = false`, the production default — Rails' reloader interlock is thread-keyed and starves under fibers). Queue pool sizes from `queues` (e.g. `mice:100`) set fiber counts when this is enabled and take precedence over `max_threads`. You can also set this with the environment variable `GOOD_JOB_FIBERS` or the CLI option `--fibers`; setting it to `0` or `false` disables fiber execution, and `true` enables it with the default count (25).
    - Only route IO-bound jobs to a fiber worker. Every fiber shares one thread, so a job that holds the CPU — rendering a PDF, resizing an image, parsing a large document — blocks *every* other job on that reactor for its whole duration, not just itself. The symptom is the entire queue slowing down with nothing in the logs to explain it, so prefer a separate thread-pool worker (a dedicated queue) for anything CPU-heavy rather than assuming a mostly-IO job is close enough. The same applies to C extensions that don't cooperate with Ruby's fiber scheduler.
    - Blast radius: a job that raises an ordinary `StandardError` is contained, and its siblings keep running. GoodJob also contains non-`StandardError` exceptions (`SystemStackError`, `NoMemoryError`, a gem raising bare `Exception`) for the same reason — left unhandled they would tear down the reactor and every job running on it, which would then be re-executed on respawn. Only Async's own teardown signals and process signals are allowed through.
    - Connection pool sizing: when `fibers` is enabled, `lock_strategy` defaults to `:skiplocked` — the `:advisory` strategy (the default otherwise) pins one connection per in-flight job for its session-level advisory lock, which caps fiber concurrency at the connection pool size. `:skiplocked` requires the v4 `lock_type` column migration; on older schemas GoodJob silently falls back to `:advisory`, so fiber deployments should ensure that migration has run. With `:skiplocked` on Rails 7.2+, jobs release their connection back to the pool between IO waits, so a small pool (5–10 connections, plus one for LISTEN/NOTIFY) can serve hundreds of fibers. On Rails 7.1 and earlier, connections may be retained for the duration of each job, so size the pool closer to the fiber count. Jobs that are themselves database-bound (long queries, transactions, or explicit `lease_connection`) hold a connection while it's in use regardless of strategy.
...
By default, GoodJob claims jobs using PostgreSQL advisory locks (except when the `fibers` option is enabled, which defaults to `:skiplocked`). As an alternative, GoodJob can use `SELECT FOR UPDATE SKIP LOCKED` to claim jobs, which writes the lock state directly to the `good_jobs` table rather than relying on session-level advisory locks.
